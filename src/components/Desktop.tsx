...
const Desktop = () => {
    ...
    const dragRef = React.useRef({ startX: 0, startY: 0, origX: 0, origY: 0, key: null, moved: false });
    ...
    const getDefaultPos = (index) => {
        const colH = Math.floor((window.innerHeight - 120) / 85);
        const col = Math.floor(index / colH);
        const row = index % colH;
        return { x: window.innerWidth - 85 - col * 90, y: 35 + row * 85 };
    };

    const getPos = (key, index) => iconPositions[key] || getDefaultPos(index);

    const handleIconMouseDown = (e, key, index) => {
        if (e.button !== 0) return;
        const pos = getPos(key, index);
        dragRef.current = { startX: e.clientX, startY: e.clientY, origX: pos.x, origY: pos.y, key, moved: false };
        setDragging(key);
    ...
    return (
        <div id="desktop-bg" className="fixed inset-0 overflow-hidden" style={wallpaperStyle} onMouseDown={handleDesktopClick}>
                    ...
                    if (item.type === 'app') {
                        ...
                        return (
                            <div key={item.key}
                                ...
                                onMouseDown={(e) => { e.stopPropagation(); setSelectedDesktopItem(item.key); handleIconMouseDown(e, item.key, index); }}
            ...
            {/* Launchpad */}
            {launchpadOpen && (
                <div className="fixed inset-0 z-[9997] flex items-center justify-center animate-fade-in"
            ...
            {/* About This Mac */}
            {aboutMacOpen && (
                <div className="fixed inset-0 z-[10004] flex items-center justify-center bg-black/30"
                    onClick={() => MacStore.setState({ aboutMacOpen: false })}>
                    <div className="glass-menu rounded-2xl shadow-2xl border border-black/10 w-[540px] overflow-hidden animate-window-open"
                        onClick={e => e.stopPropagation()}>
            ...
            <div className="fixed top-[32px] right-3 z-[10005] flex flex-col gap-2">
                {notifications.map(notif => (
                    <div key={notif.id} className="glass-menu rounded-xl shadow-xl border border-black/10 p-3 w-[320px] animate-toast-in cursor-default"
                        onClick={() => MacStore.setState(s => ({ notifications: s.notifications.filter(n => n.id !== notif.id) }))}>
