...
const ControlCenter = () => {
    ...
    const Tile = ({ active, onClick, icon, label, sublabel }) => (
        <div onClick={onClick}
            className={`rounded-xl p-3 cursor-default text-center transition-colors
                ${active ? 'bg-blue-500 text-white' : 'bg-white/60 hover:bg-white/80'}`}>
    ...
    return (
        <div className="fixed z-[10001]" style={{ top: '31px', right: '12px' }}
            ...
            <div className="w-[320px] glass-menu rounded-2xl shadow-2xl border border-black/10 p-3.5 animate-spotlight-in">
                <div className="grid grid-cols-3 gap-2.5 mb-3">
                    <Tile active={wifi} onClick={() => setWifi(!wifi)} label="Wi-Fi" sublabel={wifi ? 'Home' : 'Off'}
                        icon={<svg viewBox="0 0 24 24" width="18" height="18" fill={wifi ? 'white' : '#333'}><path d="M1 9l2 2c4.97-4.97 13.03-4.97 18 0l2-2C16.93 2.93 7.08 2.93 1 9zm8 8l3 3 3-3c-1.65-1.66-4.34-1.66-6 0zm-4-4l2 2c2.76-2.76 7.24-2.76 10 0l2-2C15.14 9.14 8.87 9.14 5 13z"/></svg>}/>
                    <Tile active={bt} onClick={() => setBt(!bt)} label="Bluetooth" sublabel={bt ? 'On' : 'Off'}
                        icon={<svg viewBox="0 0 24 24" width="18" height="18" fill={bt ? 'white' : '#333'}><path d="M17.71 7.71L12 2h-1v7.59L6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 11 14.41V22h1l5.71-5.71-4.3-4.29z"/></svg>}/>
                ...
                <div className="mb-3">
                    ...
                    <div className="flex items-center gap-2 bg-white/60 rounded-xl px-3 py-2">
                        ...
                        <input type="range" min="10" max="100" value={brightness} onChange={e => setBrightness(+e.target.value)} className="flex-1 accent-blue-500 h-1.5"/>
                ...
                <div className="mb-3">
                    ...
                    <div className="flex items-center gap-2 bg-white/60 rounded-xl px-3 py-2">
                        ...
                        <input type="range" min="0" max="100" value={volume} onChange={e => setVolume(+e.target.value)} className="flex-1 accent-blue-500 h-1.5"/>
                ...
                <div className="grid grid-cols-2 gap-2.5">
                    <div onClick={() => setFocus(!focus)}
                        className={`flex flex-col items-center gap-1.5 rounded-xl py-3 cursor-default transition-colors
                            ${focus ? 'bg-blue-500 text-white' : 'bg-white/60 hover:bg-white/80'}`}>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${focus ? 'bg-white/20' : 'bg-black/5'}`}>
                            <svg viewBox="0 0 24 24" width="16" height="16" fill={focus ? 'white' : '#333'}><path d="M9.37 5.51c-.18.64-.27 1.31-.27 1.99 0 4.08 3.32 7.4 7.4 7.4.68 0 1.35-.09 1.99-.27C17.45 17.19 14.93 19 12 19c-3.86 0-7-3.14-7-7 0-2.93 1.81-5.45 4.37-6.49z"/></svg>
                        </div>
                        <span className="text-[11px] font-medium">Focus</span>
                    </div>
                    <div onClick={() => setDarkMode(!darkMode)}
                        className={`flex flex-col items-center gap-1.5 rounded-xl py-3 cursor-default transition-colors
                            ${darkMode ? 'bg-blue-500 text-white' : 'bg-white/60 hover:bg-white/80'}`}>
